...
class CI_Captcha
{
  ...
  $this->createCode();
    ...
    private function createImg(){
        ...
        $bgColor = imagecolorallocate($this->im, 71, 67, 67);
        imagefill($this->im, 0, 0, $bgColor);
    }

    private function setDisturb(){
        $area = ($this->width * $this->height) / 10;
        $disturbNum = ($area > 500) ? 500 : $area;
        //加入点干扰
    ...
    private function setCaptcha(){
        for ($i = 0; $i < $this->codeNum; $i++) {
            $color = imagecolorallocate($this->im, 230, 221, 221);
            $size =500;
            $x = floor($this->width / $this->codeNum) * $i +2;
            $y = rand(10, $this->height - 20);
